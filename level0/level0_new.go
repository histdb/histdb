...
func (t *T2) Init(fh filesystem.File) error {
...
func (t *T2) Append(key lsm.Key, ts uint32, value []byte) (bool, error) {
...
func (t *T2) append(key lsm.Key, ts uint32, value []byte) (bool, error) {
	...
	length := 4 + 16 + 4 + uint32(len(value))
...
func (t *T2) storeErr(err error) error {
	t.err = errs.Wrap(err)
	return t.err
}
...
func (t *T2) Iterator() (it Iterator, err error) {
	if t.err != nil {
		err = t.err
	} else if !t.done {
